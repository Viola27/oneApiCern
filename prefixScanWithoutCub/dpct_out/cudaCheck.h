...
inline bool cudaCheck_(const char *file, int line, const char *cmd, int result,
  ...
  /*
      DPCT1007:1: Migration of this CUDA API is not supported by the
   * Intel(R) DPC++ Compatibility Tool.
      */
...
inline bool cudaCheck_(const char *file, int line, const char *cmd, int result,
                       const char *description = nullptr) try {
  if (LIKELY(result == 0))
        return true;

  /*
      DPCT1009:2: SYCL uses exceptions to report errors and does not use
   * the error codes. The original code was commented out and a warning string
   * was inserted. You need to rewrite this code.
      */
  const char *error =
      "cudaGetErrorName not supported" /*cudaGetErrorName(result)*/;
  /*
      DPCT1009:3: SYCL uses exceptions to report errors and does not use
   * the error codes. The original code was commented out and a warning string
   * was inserted. You need to rewrite this code.
      */
  const char *message =
