...
void __dpct_inline__ SYCL_EXTERNAL warpPrefixScan(T const *__restrict__ ci,
                                    T *__restrict__ co, uint32_t i,
                                    uint32_t mask, sycl::nd_item<3> item_ct1) {
...
template <typename T>
void __dpct_inline__ SYCL_EXTERNAL warpPrefixScan(T *c, uint32_t i, uint32_t mask,
...
__dpct_inline__ SYCL_EXTERNAL void
blockPrefixScan(VT const *ci, VT *co, uint32_t size,
                T *ws,
                    sycl::nd_item<3>
                        item_ct1
...
template <typename T>
__dpct_inline__ SYCL_EXTERNAL void
blockPrefixScan(T *c, uint32_t size,
                T *ws,
                    sycl::nd_item<3>
                        item_ct1
#ifndef DPCPP_COMPATIBILITY_TEMP 
...
__dpct_inline__ unsigned dynamic_smem_size() {
  ...
  int off = item_ct1.get_local_range().get(2) * item_ct1.get_group(2);
    ...
    blockPrefixScan(
        ci + off, co + off,
        sycl::min(int(item_ct1.get_local_range(2)), (int)(size - off)),
        ws, item_ct1);
